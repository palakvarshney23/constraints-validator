...
func main() {
	...
	if len(os.Args) > 1 {
		...
		if err := analyze(a); err != nil { fmt.Println("\n  Error:", err) }
		return
	}
	fmt.Print("\n  GitHub URL: ")
	...
	if u == "" { return }
	if err := analyze(u); err != nil { fmt.Println("\n  Error:", err) }
}
func analyze(rawURL string) error {
	...
	if m == nil { return fmt.Errorf("invalid GitHub URL") }
	...
	for i := 0; i < 4; i++ { fmt.Printf("\r  %s  %s", spin[i%4], lbl); time.Sleep(40 * time.Millisecond) }
	rq, _ := http.NewRequest("GET", "https://api.github.com/repos/"+owner+"/"+repo, nil)
	rq.Header.Set("Accept", "application/vnd.github+json"); rq.Header.Set("User-Agent", "co-check")
	if tok != "" { rq.Header.Set("Authorization", "Bearer "+tok) }
	rs, err := cl.Do(rq)
	if err != nil { return err }
	repoB, _ := io.ReadAll(rs.Body); rs.Body.Close()
	if rs.StatusCode >= 400 { return fmt.Errorf("GitHub API %s", rs.Status) }
	var meta struct {
	...
	if json.Unmarshal(repoB, &meta) != nil { return fmt.Errorf("bad repo JSON") }
	msg := fmt.Sprintf("  ✓  %s\n", lbl)
	fmt.Print(msg); rep.WriteString(msg)
	...
	rs, err = cl.Do(rq)
	if err != nil { return err }
	brB, _ := io.ReadAll(rs.Body); rs.Body.Close()
	if rs.StatusCode >= 400 { return fmt.Errorf("branch API %s", rs.Status) }
	var br struct { Commit struct { Sha string `json:"sha"` } `json:"commit"` }
	if json.Unmarshal(brB, &br) != nil || br.Commit.Sha == "" { return fmt.Errorf("bad branch JSON") }
	...
	rs, err = cl.Do(rq)
	if err != nil { return err }
	treeB, _ := io.ReadAll(rs.Body); rs.Body.Close()
	if rs.StatusCode >= 400 { return fmt.Errorf("tree API %s", rs.Status) }
	var tree struct {
		Tree []struct { Path string `json:"path"`; Type string `json:"type"`; Sha string `json:"sha"`; Size int `json:"size"` } `json:"tree"`
	}
	if json.Unmarshal(treeB, &tree) != nil { return fmt.Errorf("bad tree JSON") }
	...
	if rs2, e := cl.Do(rq); e == nil && rs2.StatusCode < 400 {
		rb, _ := io.ReadAll(rs2.Body); rs2.Body.Close()
		var rd struct { Content string `json:"content"` }
	...
	for _, ent := range tree.Tree {
		...
		pathLow.WriteByte(' ')
		if ent.Type != "blob" || !srcExt.MatchString(ent.Path) || ent.Size > 120000 { continue }
		skip := false
		for _, d := range []string{"node_modules/", "vendor/", ".git/", "dist/", "build/", "__pycache__/"} {
			if strings.Contains(ent.Path, d) { skip = true; break }
		}
		if skip { skipN++; continue }
		bq, e := http.NewRequest("GET", "https://api.github.com/repos/"+owner+"/"+repo+"/git/blobs/"+ent.Sha, nil)
		if e != nil { skipN++; continue }
		bq.Header.Set("Accept", "application/vnd.github.raw"); bq.Header.Set("User-Agent", "co-check")
		if tok != "" { bq.Header.Set("Authorization", "Bearer "+tok) }
		brs, e := cl.Do(bq)
		if e != nil || brs.StatusCode >= 400 { if brs != nil { brs.Body.Close() }; skipN++; continue }
		body, e := io.ReadAll(brs.Body); brs.Body.Close()
		if e != nil { skipN++; continue }
	...
	code := buf.String()
	if strings.TrimSpace(code) == "" { return fmt.Errorf("no source files found") }
	...
	for _, ln := range strings.Split(strings.ReplaceAll(code, "\r\n", "\n"), "\n") {
		...
		if t == "" { continue }
		if inBlock { if strings.Contains(t, "*/") { inBlock = false }; continue }
		if strings.HasPrefix(t, "/*") { if idx := strings.Index(t, "*/"); idx < 0 || idx <= 2 { inBlock = true }; continue }
		if strings.HasPrefix(t, "//") || strings.HasPrefix(t, "#") || strings.HasPrefix(t, "--") ||
			strings.HasPrefix(t, "<!--") || strings.HasPrefix(t, "*") || strings.HasPrefix(t, ";") { continue }
		codeLines++
	}
	...
	for _, f := range regexp.MustCompile(`(?m)\bfunction\s+([A-Za-z_]\w*)|\bdef\s+([A-Za-z_]\w*)`).FindAllStringSubmatch(code, -1) {
		n := f[1]; if n == "" { n = f[2] }; if n != "" && n != "main" { fnSet[n] = true }
	}
	for _, f := range regexp.MustCompile(`(?m)^func\s+([A-Za-z_]\w*)\s*\(`).FindAllStringSubmatch(code, -1) {
		if f[1] != "main" && f[1] != "init" { fnSet[f[1]] = true }
	}
	for _, f := range regexp.MustCompile(`(?m)^func\s+\([^)]+\)\s+([A-Za-z_]\w*)\s*\(`).FindAllStringSubmatch(code, -1) { fnSet[f[1]] = true }
	for _, f := range regexp.MustCompile(`(?m)^\s+(?:public|private|protected|static|final|override)\s+\S+\s+([A-Za-z_]\w*)\s*\(`).FindAllStringSubmatch(code, -1) {
		if f[1] != "main" { fnSet[f[1]] = true }
	}
	for _, f := range regexp.MustCompile(`(?m)(?:const|let|var)\s+([A-Za-z_]\w*)\s*=\s*(?:async\s*)?\(`).FindAllStringSubmatch(code, -1) { fnSet[f[1]] = true }
	fnCount := len(fnSet)
	loops := len(regexp.MustCompile(`\b(for|while|do)\s*[({]`).FindAllString(code, -1))
	varSet := map[string]bool{}
	for _, v := range regexp.MustCompile(`(?m)^\s+([a-z][A-Za-z0-9]*)\s*:=`).FindAllStringSubmatch(code, -1) { varSet[v[1]] = true }
	for _, v := range regexp.MustCompile(`(?m)\b(?:let|const)\s+([A-Za-z_]\w*)`).FindAllStringSubmatch(code, -1) { varSet[v[1]] = true }
	for _, v := range regexp.MustCompile(`(?m)^\s{4}([a-z_][a-z0-9_]*)\s*=[^=<>!]`).FindAllStringSubmatch(code, -1) { varSet[v[1]] = true }
	varCount := len(varSet)
	shortFail := false
	for n := range varSet { if len(n) > 3 { shortFail = true; break } }
	...
	if topLang == "" {
		best, bestN := "", 0
		for ext, n := range langCnt { if n > bestN { bestN = n; best = ext } }
		if best != "" { topLang = extMap[best]; if topLang == "" { topLang = strings.ToUpper(best) } }
	}
	if topLang == "" { topLang = "n/a" }
	...
	fmt.Print(msg); rep.WriteString(msg)
	d1Pass := []string{}
	if imports == 0 { d1Pass = append(d1Pass, "No-Import Rookie") }
	if varCount <= 8 { d1Pass = append(d1Pass, "Few-Variable Hero") }
	if fnCount <= 1 { d1Pass = append(d1Pass, "Single-Function Master") }
	if errOk { d1Pass = append(d1Pass, "Error-Proof Coder") }
	if loops <= 1 { d1Pass = append(d1Pass, "One-Loop Warrior") }
	if !shortFail { d1Pass = append(d1Pass, "Short-Name Ninja") }
	if fastOk { d1Pass = append(d1Pass, "Fast-Response Builder") }
	if stateOk { d1Pass = append(d1Pass, "Simple-State Creator") }
	budgets := []struct{ name string; max int }{
	...
	for _, b := range budgets {
		if codeLines <= b.max { d2Pass = append(d2Pass, fmt.Sprintf("%s (%d lines)", b.name, codeLines)); break }
	}
	if readmeLow != "" { metaHigh += " " + readmeLow }
	codeLow := strings.ToLower(code)
	paths := pathLow.String()
	domains := []struct{ name string; keys []string }{
	...
	type d3Rank struct{ name string; score int }
	ranks := []d3Rank{}
	for _, d := range domains {
		...
		if sc > 0 { ranks = append(ranks, d3Rank{d.name, sc}) }
	}
	for i := 0; i < len(ranks); i++ {
		for j := i + 1; j < len(ranks); j++ {
			if ranks[j].score > ranks[i].score { ranks[i], ranks[j] = ranks[j], ranks[i] }
		}
	}
	d3Total := 0
	for _, r := range ranks { d3Total += r.score }
	d3Pass := []string{}
	for i := 0; i < len(ranks) && i < 3; i++ {
		pct := 0
		if d3Total > 0 { pct = ranks[i].score * 100 / d3Total }
		d3Pass = append(d3Pass, fmt.Sprintf("%s (%d, %d%%)", ranks[i].name, ranks[i].score, pct))
	}
	d4Pass := []string{}
	if topLang != "n/a" { d4Pass = append(d4Pass, topLang) }
	tables := []struct{ title string; items []string }{
	...
	fmt.Print(sum); rep.WriteString(sum)
	total := 0
	for _, tb := range tables {
		fmt.Println(); fmt.Println("  " + tb.title); rep.WriteString("\n  " + tb.title + "\n")
		if len(tb.items) == 0 {
			fmt.Println("  +------------------------------------------------------------------+")

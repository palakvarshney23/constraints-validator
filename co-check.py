#!/usr/bin/env python3
"""Code Olympics GitHub Constraint Auditor — Python Rosetta Stone.

Uses only the Python standard library. Demonstrates how the same
problem produces a radically different architecture when a language
encourages decomposition and exception-based flow.
"""
import json
import os
import re
...
def analyze(owner: str, repo: str, token: str):
    ...
    for ent in tree:
        ...
        langcnt[ext] = langcnt.get(ext, 0) + 1
        parts.append(body)
        fcount += 1
        if fcount >= MAXF:
            break
    ...
    fns = len(
        set(
            m[0] or m[1]
            for m in re.findall(r'(?m)\bfunction\s+([A-Za-z_]\w*)|\bdef\s+([A-Za-z_]\w*)', code)
        )
    )
...
def main():
    print("╔═══════════════════════════════════════════════════════╗")
    print("║  Code Olympics -- GitHub Constraint Auditor (Python)  ║")
    print("╚═══════════════════════════════════════════════════════╝")
    u = input("\nGitHub URL (or 'q' to quit): ").strip()
    if not u or u.lower() == "q":
        return
